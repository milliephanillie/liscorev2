...
import MyEmailSubscriptions from './MyEmailSubscriptions';
import MySavedSearches from './MySavedSearches';

const MySubscriptionsTabs = (props) => {
  const { productId } = props;
  const data = useSelector(state => state);
  const { loading, business, product } = data;
  const toScroll = useRef(null);

  let url = `${lc_data.myaccount}subscriptions`;
  ...
  return [
    <div key={0} className="w-full bg:w-11/16">
        ...
        <ScrollContainer ref={toScroll}>
          <ul className="flex items-center w-full">
            ...
            <li>
              <NavLink
                exact
                to={`${lc_data.site_url}${url}/email-subscriptions`}
                className="flex-center py-4 px-20 rounded font-bold text-grey-900 text-lg whitespace-no-wrap"
                activeClassName="bg-grey-100"
              >
                {lc_data.jst[636]}
            ...
            {data.options?.save_searches &&
            <li>
              <NavLink
                exact
                to={`${lc_data.site_url}${url}/saved-searches`}
                className="flex-center py-4 px-20 rounded font-bold text-grey-900 text-lg whitespace-no-wrap"
                activeClassName="bg-grey-100"
              >
                {lc_data.jst[770]}
      ...
      <Route exact path={`${lc_data.site_url}${url}`} component={() => <MyBookmarks/>}/>
      <Route exact path={`${lc_data.site_url}${url}/email-subscriptions`} component={() => <MyEmailSubscriptions/>}/>
      {data.options?.save_searches &&
      <Route exact path={`${lc_data.site_url}${url}/saved-searches`} component={() => <MySavedSearches/>}/>
      }
...
export default MySubscriptionsTabs;
