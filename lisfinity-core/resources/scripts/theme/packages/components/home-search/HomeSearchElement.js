...
class HomeSearchElement extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  componentDidMount() {
    ...
    const wrapper = this.el.current.closest('.home-search');
  ...
  render() {
    ...
    if (this.state.options.selected_icon_button !== null && this.state.options.selected_icon_button !== undefined) {
      typeof this.state.options.selected_icon_button['value'] === 'string' ? keyboardIcon = this.state.options.selected_icon_button['value'] : keyboardSvg = this.state.options.selected_icon_button['value']['url'];
    }

    let fieldWidth = null;
    let fieldGap = null;
    let buttonIndex = null;
    options.order?.forEach((el, index) => {
      if ('hero_button' === el) {
        buttonIndex = index;
      }
    });
    ...
    if (buttonIndex) {
      fieldWidth = options['lisfinity_hero_form_fields'][buttonIndex]['width']?.size && options['lisfinity_hero_form_fields'][buttonIndex]['width']?.unit ? `${options['lisfinity_hero_form_fields'][buttonIndex]['width'].size}${options['lisfinity_hero_form_fields'][buttonIndex]['width'].unit}` : '50%';
      fieldGap = options['lisfinity_hero_form_fields'][buttonIndex]['gap']?.size && options['lisfinity_hero_form_fields'][buttonIndex]['gap']?.unit ? `${options['lisfinity_hero_form_fields'][buttonIndex]['gap'].size}${options['lisfinity_hero_form_fields'][buttonIndex]['gap'].unit}` : '0px';
    }
    ...
    return (
      <div ref={this.el}>
        ...
        <form key={1} action={lc_data.page_search}
              className={`relative search--form ${fieldsStyle === '2' ? 'flex flex-wrap' : 'flex-center flex-wrap -mb-10'}`}
              style={{
        ...
        >
          ...
          <Keyword keywordValueHandle={(value) => this.keywordValueHandle(value)} fields={fields}
                   searchValues={keyword}
                   options={options}/>
          }
          {!isEmpty(taxonomy) && !isEmpty(terms) &&
          <HomeTaxonomy className={`w-full`} fields={fields} taxonomy={taxonomy} terms={terms} type="home"
                        options={options} columns={this.state.columns}
          ...
          <div
            className={`mt-10 w-full flex hero-button-container lisfinity-order-${buttonIndex + 1}`}
            style={{
          ...
          >
            ...
            >
              ...
              {(!loading || !this.state.waiting) && !this.state.waiting && ((keyboardSvg === null && keyboardIcon === null) || (!keyboardSvg && !keyboardIcon)) && options.remove_icon_button !== 'yes' &&
              <Fragment>
              ...
              {(!loading || !this.state.waiting) && !this.state.waiting && ((keyboardSvg === null && keyboardIcon === null) || (!keyboardSvg && !keyboardIcon)) && options.remove_text_button !== 'yes' &&
              <Fragment>
                <span>{options.button_text}</span>
              </Fragment>
              }
              {
                (!loading || !this.state.waiting) && !this.state.waiting && keyboardSvg && this.state.options?.selected_icon_button !== '' && options.remove_icon_button !== 'yes' &&
                <Fragment>
                  <img src={keyboardSvg} alt="cart-icon" id="button-icon"
                       className="w-20 h-20 fill-icon-reset pointer-events-none"/>
                </Fragment>
              }
              {
                (!loading || !this.state.waiting) && !this.state.waiting && keyboardSvg && this.state.options?.selected_icon_button !== '' && options.remove_text_button !== 'yes' &&
                <Fragment>
                  <span>{options.button_text}</span>
                </Fragment>
              }
              {
                (!loading || !this.state.waiting) && !this.state.waiting && this.state.options?.selected_icon_button !== '' && keyboardIcon && options.remove_icon_button !== 'yes' &&
                <Fragment>
                  <i className={keyboardIcon} id="button-icon" aria-hidden="true"></i>
                </Fragment>
              }
              {
                (!loading || !this.state.waiting) && !this.state.waiting && keyboardIcon && this.state.options?.selected_icon_button !== '' && options.remove_text_button !== 'yes' &&
                <Fragment>
                  <span>{options.button_text}</span>
                </Fragment>
              }
...
export default connect(mapStateToProps)(HomeSearchElement);
