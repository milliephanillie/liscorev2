...
const BusinessStoreBreadcrumbEl = (props) => {
  ...
  let chosenSortOptions = [];
  if (props?.options && props?.options?.ads_sortby) {
    chosenSortOptions = props?.options?.ads_sortby;
  }
  const sortOptions = [];
        ...
        <ul className="flex items-center -mx-4">
          ...
          <li className="ml-auto">
            <div className="search--action flex items-center">
              <label htmlFor="sortby" className="flex items-center mr-4 search-filter-top-label text-sm text-grey-500">
                {(!settings?.filter_icon || settings?.filter_icon.value === '') &&
                <ReactSVG src={`${lc_data.dir}dist/${sortIcon}`} className="mr-10 -ml-10 w-18 h-18 fill-icon-search filter-icon"/>
                }
                {settings?.filter_icon?.value && settings?.filter_icon?.library !== 'svg' &&
                <i className={`mr-8 ${settings.filter_icon.value} fill-icon-search filter-icon`}></i>
                }
                {settings?.filter_icon?.value.url && settings?.filter_icon?.library === 'svg' &&
                <ReactSVG
                  src={`${settings.filter_icon.value.url}`}
                  className={`mr-8 w-16 h-16 fill-icon-search filter-icon`}
                />
                }
                {lc_data.jst[182]}
              </label>
...
export default BusinessStoreBreadcrumbEl;
