...
import {createRef, Fragment, useEffect, useState} from '@wordpress/element';
...
const BusinessTestimonialEl = (props) => {
  ...
  useEffect(() => {
    wrapper = el.current.closest('.elementor-business-testimonial');
  ...
  useEffect(() => {
    setMaxLength(options.reviews_length);
  ...
  let buttonText = options && options?.text ? options.display_text : lc_data.jst[409];

  return (
    <div className={`business--testimonial mt-10 w-full `} ref={el}>
      {options.reviews && product?.premium_profile?.testimonial?.options && !isEmpty(product?.premium_profile?.testimonial?.options) &&
      <Fragment>
        {lc_data.logged_in &&
        <button
          type="button"
          className="flex-center py-8 px-30 w-full border border-yellow-700 rounded testimonial-button font-bold text-yellow-700 whitespace-no-wrap"
          onClick={() => setModalOpen(true)}
        >
          {((!options?.icon || options?.icon.value === '') && options.display_icon === '') &&
          <ReactSVG
            src={`${lc_data.dir}dist/${StarIcon}`}
            className="relative mr-12 w-16 h-16 fill-yellow-700 testimonial-icon"
          />}
          {options?.icon?.value && options?.icon?.library !== 'svg' &&
          <i className={`${options.icon.value} relative mr-12 w-16 h-16 fill-yellow-700 testimonial-icon`}></i>
          }
          {options?.icon?.value.url && options?.icon?.library === 'svg' &&
          <ReactSVG
            src={`${options.icon.value.url}`}
            className={`relative mr-12 w-16 h-16 fill-yellow-700 testimonial-icon`}
          />
          }
      ...
      >
        ...
        >
          ...
          {lc_data.logged_in && product.author != lc_data.current_user_id && options.reviews !== '0' &&
          <SendTestimonial
            product={product}
...
export default BusinessTestimonialEl;
