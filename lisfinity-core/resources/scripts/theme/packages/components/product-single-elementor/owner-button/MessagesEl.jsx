...
import MessagesList from '../../product-single/messages/MessagesList';
...
class MessagesEl extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  render() {
    ...
    const owner = product?.premium_profile?.owner;
    ...
    if (this.props.settings?.selected_icon_messages_button !== null && this.props.settings?.selected_icon_messages_button) {
      typeof this.props.settings.selected_icon_messages_button['value'] === 'string' ? icon = this.props.settings.selected_icon_messages_button['value'] : svg = this.props.settings.selected_icon_messages_button['value']['url'];
    }

    let iconListingOwner = null;
    let svgListingOwner = null;

    if (this.props.settings?.selected_icon_messages_button_listing_owner !== null && this.props.settings?.selected_icon_messages_button_listing_owner) {
      typeof this.props.settings.selected_icon_messages_button_listing_owner['value'] === 'string' ? iconListingOwner = this.props.settings.selected_icon_messages_button_listing_owner['value'] : svgListingOwner = this.props.settings.selected_icon_messages_button_listing_owner['value']['url'];
    }

    return [
      <div key={0} className="send-message w-48% bg:w-full xl:w-48% xl:pl-4">
        {owner == currentUser.ID && lc_data.is_demo &&
        <button type="button"
                className={`relative flex-center messages-button-listing-owner btn btn--transparent-red font-normal ${btnClass} px-24 whitespace-no-wrap w-full`}
                onClick={() => this.openForm()}
        >{
          svgListingOwner && this.props.settings.place_icon_messages_button_listing_owner !== '' &&
          <img src={svgListingOwner} alt="cart-icon"
               className="fill-icon-contact messages-icon-listing-owner"/>
        }
          {
            this.props.settings.place_icon_messages_button_listing_owner !== '' && iconListingOwner &&
            <i className={`${iconListingOwner} messages-icon-listing-owner`} style={{
              display: 'flex',
              alignSelf: 'center'
            }}
               aria-hidden="true"
        ...
        <a href={readMessagesLink} onClick={this.openForm}
           className="relative flex-center btn messages-button-listing-owner btn--transparent-red font-normal whitespace-no-wrap w-full">
        ...
        {lc_data.logged_in && owner != currentUser.ID &&
        <button type="button"
                className={`relative flex-center messages-button btn btn--transparent-red font-normal ${btnClass} px-24 whitespace-no-wrap w-full`}
                onClick={() => {
                  this.openForm();
        ...
        >
          {
            svg && this.props.settings.place_icon_messages_button !== '' &&
            <img src={svg} alt="cart-icon"
                 className="fill-icon-contact messages-icon"/>
          }
          {
            this.props.settings.place_icon_messages_button !== '' && icon &&
            <i className={`${icon} messages-icon`} style={{
              display: 'flex',
              alignSelf: 'center'
            }}
               aria-hidden="true"
        ...
        >
            <span
              className="absolute text-not-owner-button left-10 px-4 bg-white text-sm text-grey-500">{lc_data.jst[500]}</span>
          {(icon === null && svg === null || "" == icon) &&
          <ReactSVG
            src={`${lc_data.dir}dist/${CommentsIcon}`}
            className="messages-icon fill-icon-home"
          />
          }
          {
            svg && this.props.settings.place_icon_messages_button !== '' &&
      ...
      owner == currentUser.ID && lc_data.is_demo && formOpen &&
      <div
        key={11}
